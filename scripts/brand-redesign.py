...
mark=''.join(f'<path d="{d}" fill="currentColor"/>' for d in paths)
fonts={n:TTFont(fontdir/(n+'.ttf')) for n in ['newsreader-regular','newsreader-italic','inter-regular','inter-medium','inter-semibold']}
def text(value,x,y,size,font='inter-regular',fill=ink,spacing=0):
...
def wordmark(x,y,scale=1,color=wine):
 return f'<g transform="translate({x} {y}) scale({scale})" color="{color}">{mark}</g>'+text('Beyond Fluency',x+60*scale,y+25*scale,27*scale,'newsreader-regular',color)+text('Lab',x+61*scale,y+46*scale,18*scale,'newsreader-italic',color)
...
card=f'<rect width="850" height="550" fill="{paper}"/>'+wordmark(55,42,1.9)+f'<path d="M55 174H795" stroke="{line}"/>'+text('[Full name]',55,256,40,'newsreader-regular')+text('[Role / title]',55,292,16,'inter-medium',wine)+text('[Email address]  ·  [Phone number]',55,390,16)+text('[Verified website address]',55,421,16)+text('FLUENT IS ONLY THE BEGINNING.',55,497,10,'inter-medium',wine,1.7)
(p/'business-card.svg').write_text(svg(card,850,550).replace('width="850" height="550"','width="85mm" height="55mm"'))
back=f'<rect width="850" height="550" fill="{wine}"/><g transform="translate(353 83) scale(3)" color="{paper}">{mark}</g>'+text('Fluent is only',190,337,54,'newsreader-regular',paper)+text('the beginning.',176,400,54,'newsreader-italic',paper)+text('BEYOND FLUENCY LAB',287,486,11,'inter-medium',paper,2)
(p/'business-card-back.svg').write_text(svg(back,850,550).replace('width="850" height="550"','width="85mm" height="55mm"'))
letter=f'<rect width="794" height="1123" fill="{paper}"/>'+wordmark(65,50,1.45)+f'<path d="M65 150H729" stroke="{line}"/>'+text('[Date]',65,213,13)+text('[Recipient name]',65,273,13)+text('[Organisation / address]',65,297,13)+text('[Subject]',65,365,32,'newsreader-regular')+text('Dear [Name],',65,420,14)+text('[Letter content]',65,469,14)+text('Kind regards,',65,830,14)+text('[Name and role]',65,867,14)+f'<path d="M65 1000H729" stroke="{wine}"/>'+text('Fluent is only the beginning.',65,1030,18,'newsreader-italic',wine)+text('[Legal entity] · [Registered address] · [Registration number]',65,1058,9)+text('[Email] · [Phone] · [Website] · VAT ID: [TO BE CONFIRMED]',65,1078,9)
...
html=(p/'letterhead.html').read_text().replace('#572B39',wine).replace('#F3F0EA',paper);html=html.replace('#f2f0eb',paper).replace('#292d29',ink).replace('#99563e',wine).replace('#d5d4cc',line).replace('font:12pt/1.6 Arial','font:11pt/1.8 Inter,Arial').replace('font:22pt Georgia','font:24pt "Newsreader",Georgia').replace('font:12pt Georgia','font:14pt "Newsreader",Georgia')
css='@font-face{font-family:Inter;src:url(fonts/inter-regular.woff)}@font-face{font-family:"Newsreader";src:url(fonts/newsreader-regular.woff)}'
...
icons=dict(re.findall(r"^ (\w+):'([^']+)'",source,re.M));parts=[f'<rect width="1000" height="{160+((len(icons)+5)//6)*140}" fill="{paper}"/>',text('The Lab iconography',35,65,36,'newsreader-regular',wine),text('One stroke. A shared visual language.',37,100,13)]
